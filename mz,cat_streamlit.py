...
CENTER_COORDS_str = input("Enter the center coordinates (Latitude,Longitude): ")
# Split the input string and convert to a tuple of floats
CENTER_COORDS = tuple(map(float, CENTER_COORDS_str.split(',')))

# --- Configuration ---
z = input("Enter the average roof height of the structure (m): ")
z = float(z) # Convert the input string to a float
LAG_DISTANCE_METERS = 20*z # Fig. 4.1 AS1170.2
print(f"Lag Distance: {LAG_DISTANCE_METERS}m")
print(f"Averaging Distance: {max(500, 40*z)}m")
...
folium.Circle(
    ...
    popup=f'Lag Distance Zone ({LAG_DISTANCE_METERS}m)',
    name='Lag Zone Mask'
).add_to(m)
